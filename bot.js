...
if (process.env.DEBUG) {
  ...
  let tunnel = localtunnel(4747, { subdomain: 'slacklatex' }, function(err, t) {
    ...
    console.log("SlackLatex's /texify available on " + t.url + '/texify');
  });
...
function getWebSocket() {
  request(
    'https://slack.com/api/rtm.start?token=' + global.token + '&pretty=1',
...
function postLatex(user, channel, text) {
  let urlBase =
    'http://latex.codecogs.com/png.latex?%5Cdpi%7B300%7D%20' +
    encodeURIComponent(text);

  let dURL =
    'https://slack.com/api/chat.postMessage?as_user=true&token=' +
    global.token +
    '&channel=' +
    channel +
    '&text=%20&attachments=%5B%7B%22fallback%22%3A%22.%22%2C%22color%22%3A%20%22%2336a64f%22%2C%22image_url%22%3A%22' +
...
function updateLatex(channel, text, ts) {
  ...
  let dURL =
    'https://slack.com/api/chat.update?as_user=true&ts=' +
    ts +
    '&token=' +
    global.token +
    '&channel=' +
...
function handleMessage(mObj, message) {
  ...
  if (mObj.type === 'message' && mObj.subtype === undefined) {
    ...
    if (
      replies[mObj.message.user + mObj.channel] !== undefined &&
      replies[mObj.message.user + mObj.channel].orignal_ts === mObj.message.ts
    ) {
      if (laText != null) {
        // is latex
        updateLatex(
    ...
    if (
      replies[mObj.previous_message.user + mObj.channel] !== undefined &&
      replies[mObj.previous_message.user + mObj.channel].orignal_ts ===
        mObj.previous_message.ts
    ) {
      deleteMessage(
        replies[mObj.previous_message.user + mObj.channel].reply_ts,
        mObj.channel
      );
...
function texify(req, res) {
  let latex = getLatex(req.body.text);
  res.status(200).send();
